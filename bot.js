...
let emreguild = client.guilds.get(`765458594501754912`)
let emreonline = emreguild.members.filter(m => !m.user.bot && m.user.presence.status !== "offline").size;
let emreonliine = `Toplam Üye: ${emreguild.members.size}\nAktif Üye: ${emreonline}`

setInterval(() => {
client.channels.get(`765458594501754914`).setTopic(`${emreonliine.replace(`1`, ` :one: `).replace(/2/, ` :two: `).replace(`3`, ` :three: `).replace(/4/, ` :four: `).replace(`5`, ` :five: `).replace(/6/, ` :six: `).replace(`7`, ` :seven: `).replace(/8/, ` :eight: `).replace(`9`, ` :nine: `).replace(/0/, ` :zero: `)}`) 
}, 3000);  })
///////////
setInterval(() => {
  client.channels.get("765458594501754914").sendEmbed(
    new Discord.RichEmbed()
  .setColor('GREEN')
  .setTitle  ("Sosyal Medya Hesaplarımız")
  .setDescription()
  .addField("Sosyal Medya Hesapları:", "[Youtube](https://www.youtube.com/channel/UCqrkR5CwK1ekeB3pP0jzPPQ?view_as=subscriber)\n [Discord](https://discord.gg/VyHJK2f)\n")
